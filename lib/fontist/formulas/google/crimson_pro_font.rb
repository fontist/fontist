module Fontist
  module Formulas
    class CrimsonProFont < FontFormula
      FULLNAME = "Crimson Pro".freeze
      CLEANNAME = "CrimsonPro".freeze

      desc FULLNAME
      homepage "http://www.baronvonfonthausen.com/"

      resource "#{CLEANNAME}.zip" do
        url "https://fonts.google.com/download?family=Crimson%20Pro"
      ...
      test do
        case platform
        when :macos
          assert_predicate "$HOME/Library/Fonts/#{CLEANNAME}/CrimsonPro[wght].ttf", :exist?
        when :linux
          assert_predicate "/usr/share/fonts/truetype/#{CLEANNAME.downcase}/CrimsonPro[wght].ttf", :exist?
        end
      end
