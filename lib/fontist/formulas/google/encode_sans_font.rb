module Fontist
  module Formulas
    class EncodeSansFont < FontFormula
      FULLNAME = "Encode Sans".freeze
      CLEANNAME = "EncodeSans".freeze

      desc FULLNAME
      homepage "http://www.impallari.com"

      resource "#{CLEANNAME}.zip" do
        url "https://fonts.google.com/download?family=Encode%20Sans"
      ...
      test do
        case platform
        when :macos
          assert_predicate "$HOME/Library/Fonts/#{CLEANNAME}/EncodeSans[wdth,wght].ttf", :exist?
        when :linux
          assert_predicate "/usr/share/fonts/truetype/#{CLEANNAME.downcase}/EncodeSans[wdth,wght].ttf", :exist?
        end
      end
