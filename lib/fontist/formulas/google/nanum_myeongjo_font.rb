module Fontist
  module Formulas
    class NanumMyeongjoFont < FontFormula
      FULLNAME = "Nanum Myeongjo".freeze
      CLEANNAME = "NanumMyeongjo".freeze

      desc FULLNAME
      homepage "http://www.nhncorp.com"

      resource "#{CLEANNAME}.zip" do
        url "https://fonts.google.com/download?family=Nanum%20Myeongjo"
      ...
      test do
        case platform
        when :macos
          assert_predicate "$HOME/Library/Fonts/#{CLEANNAME}/NanumMyeongjo-Regular.ttf", :exist?
        when :linux
          assert_predicate "/usr/share/fonts/truetype/#{CLEANNAME.downcase}/NanumMyeongjo-Regular.ttf", :exist?
        end
      end
