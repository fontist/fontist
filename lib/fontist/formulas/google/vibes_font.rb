module Fontist
  module Formulas
    class VibesFont < FontFormula
      FULLNAME = "Vibes".freeze
      CLEANNAME = "Vibes".freeze

      desc FULLNAME
      homepage ""

      resource "#{CLEANNAME}.zip" do
        url "https://fonts.google.com/download?family=Vibes"
      ...
      test do
        case platform
        when :macos
          assert_predicate "$HOME/Library/Fonts/#{CLEANNAME}/Vibes-Regular.ttf", :exist?
        when :linux
          assert_predicate "/usr/share/fonts/truetype/#{CLEANNAME.downcase}/Vibes-Regular.ttf", :exist?
        end
      end
