module Fontist
  module Formulas
    class ArefRuqaaFont < FontFormula
      FULLNAME = "Aref Ruqaa".freeze
      CLEANNAME = "ArefRuqaa".freeze

      desc FULLNAME
      homepage ""

      resource "#{CLEANNAME}.zip" do
        url "https://fonts.google.com/download?family=Aref%20Ruqaa"
      ...
      test do
        case platform
        when :macos
          assert_predicate "$HOME/Library/Fonts/#{CLEANNAME}/ArefRuqaa-Regular.ttf", :exist?
        when :linux
          assert_predicate "/usr/share/fonts/truetype/#{CLEANNAME.downcase}/ArefRuqaa-Regular.ttf", :exist?
        end
      end
