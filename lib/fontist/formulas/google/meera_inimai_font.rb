module Fontist
  module Formulas
    class MeeraInimaiFont < FontFormula
      FULLNAME = "Meera Inimai".freeze
      CLEANNAME = "MeeraInimai".freeze

      desc FULLNAME
      homepage "http://indicproject.org"

      resource "#{CLEANNAME}.zip" do
        url "https://fonts.google.com/download?family=Meera%20Inimai"
      ...
      test do
        case platform
        when :macos
          assert_predicate "$HOME/Library/Fonts/#{CLEANNAME}/MeeraInimai-Regular.ttf", :exist?
        when :linux
          assert_predicate "/usr/share/fonts/truetype/#{CLEANNAME.downcase}/MeeraInimai-Regular.ttf", :exist?
        end
      end
