module Fontist
  module Formulas
    class GrenzeGotischFont < FontFormula
      FULLNAME = "Grenze Gotisch".freeze
      CLEANNAME = "GrenzeGotisch".freeze

      desc FULLNAME
      homepage "www.omnibus-type.com"

      resource "#{CLEANNAME}.zip" do
        url "https://fonts.google.com/download?family=Grenze%20Gotisch"
      ...
      test do
        case platform
        when :macos
          assert_predicate "$HOME/Library/Fonts/#{CLEANNAME}/GrenzeGotisch[wght].ttf", :exist?
        when :linux
          assert_predicate "/usr/share/fonts/truetype/#{CLEANNAME.downcase}/GrenzeGotisch[wght].ttf", :exist?
        end
      end
