...
RSpec.describe Fontist::Resources::GoogleResource do
  describe "#font_urls (private)" do
    ...
    context "with v5 basename files (files != urls)" do
      let(:resource) do
        double(
          "Resource",
      ...
      it "returns download URLs for matching basenames" do
      ...
      it "returns multiple URLs for multiple source names" do
        urls = google_resource.send(:font_urls, ["Roboto-Regular.ttf", "Roboto-Bold.ttf"])
        expect(urls).to eq([
          "https://fonts.gstatic.com/s/roboto/v30/Roboto-Regular.ttf",
          "https://fonts.gstatic.com/s/roboto/v30/Roboto-Bold.ttf",
        ])
      ...
      it "returns empty when no basenames match" do
        urls = google_resource.send(:font_urls, ["NonExistent.ttf"])
