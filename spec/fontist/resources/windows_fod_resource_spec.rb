...
RSpec.describe Fontist::Resources::WindowsFodResource do
    ...
    context "when capability is already installed" do
      before do
        allow(Fontist::Utils::System).to receive(:run_powershell)
          ...
          .and_return(Fontist::Utils::System::PowerShellResult.new(
                        stdout: "Installed\n", stderr: "", success: true,
                      ))
      end
  ...
  describe "#ps_escape" do
    let(:cap) { "Language.Fonts.Jpan~~~und-JPAN~0.0.1.0" }

    it "does not alter strings without single quotes" do
      escaped = fod_resource.send(:ps_escape, cap)
      expect(escaped).to eq(cap)
    end

    it "doubles single quotes within a string" do
      escaped = fod_resource.send(:ps_escape, "it's a test'string")
      expect(escaped).to eq("it''s a test''string")
    end
  end

  describe "#system_fonts_dir" do
    it "uses windir env var when set" do
      allow(ENV).to receive(:[]).and_call_original
      allow(ENV).to receive(:[]).with("windir").and_return("D:/Windows")
      allow(ENV).to receive(:[]).with("SystemRoot").and_return(nil)

      expect(fod_resource.send(:system_fonts_dir)).to eq("D:/Windows/Fonts")
    end

    it "falls back to SystemRoot when windir is not set" do
      allow(ENV).to receive(:[]).and_call_original
      allow(ENV).to receive(:[]).with("windir").and_return(nil)
      allow(ENV).to receive(:[]).with("SystemRoot").and_return("E:/Win")

      expect(fod_resource.send(:system_fonts_dir)).to eq("E:/Win/Fonts")
    end

    it "falls back to C:/Windows when neither env var is set" do
      allow(ENV).to receive(:[]).and_call_original
      allow(ENV).to receive(:[]).with("windir").and_return(nil)
      allow(ENV).to receive(:[]).with("SystemRoot").and_return(nil)

      expect(fod_resource.send(:system_fonts_dir)).to eq("C:/Windows/Fonts")
    end
  end
end
