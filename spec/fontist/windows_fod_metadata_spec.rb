...
RSpec.describe Fontist::WindowsFodMetadata do
  ...
  describe ".metadata" do
    ...
    it "raises Errno::ENOENT when data file is missing" do
      stub_const("Fontist::WindowsFodMetadata::DATA_PATH",
                 "/nonexistent/file.yml")
      expect { described_class.metadata }.to raise_error(Errno::ENOENT)
  ...
  describe ".reset_cache" do
    it "clears cached metadata and reverse map" do
      described_class.metadata
      described_class.capability_for_font("Meiryo")

      described_class.reset_cache

      expect(described_class.instance_variable_get(:@metadata)).to be_nil
      expect(described_class.instance_variable_get(:@reverse_map)).to be_nil
    end
  end
end
