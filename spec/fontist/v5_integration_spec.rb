...
RSpec.describe "V5 Integration" do
  ...
  describe "v5 formula loading" do
    ...
    it "loads css_url on woff2 resources" do
      woff2_static = formula.resources.find { |r| r.name == "woff2_static" }
      expect(woff2_static.css_url).to eq("https://fonts.googleapis.com/css2?family=Roboto")

      woff2_variable = formula.resources.find { |r| r.name == "woff2_variable" }
      expect(woff2_variable.css_url).to eq("https://fonts.googleapis.com/css2?family=Roboto")
    end
    ...
    it "does not have css_url on ttf resources" do
      ttf_resource = formula.resources.find { |r| r.name == "ttf_static" }
      expect(ttf_resource.css_url).to be_nil
    end
  end
