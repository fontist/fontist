...
RSpec.describe "Fontist::Import::CreateFormula" do
  ...
  context "msi archive" do
    let(:url) { "http://ardownload.adobe.com/pub/adobe/reader/win/AcrobatDC/misc/FontPack1902120058_XtdAlf_Lang_DC.msi" } # rubocop:disable Layout/LineLength
    let(:options) { { name: "Adobe Reader 19" } } # rubocop:disable Layout/LineLength
  ...
  context "7z archive" do
    let(:url) { "http://ardownload.adobe.com/pub/adobe/reader/win/AcrobatDC/2001220041/AcroRdrDC2001220041_en_US.exe" } # rubocop:disable Layout/LineLength
  ...
  context "with different SHA256 mirrors" do
    let(:url) { "https://gitlab.com/fontmirror/archive/-/raw/master/VistaFont_KOR.EXE" } # rubocop:disable Layout/LineLength
    let(:options) { { mirror: ["https://download.microsoft.com/download/0/3/e/03e8f61e-be04-4cbd-8007-85a544fec76b/VistaFont_KOR.EXE"] } } # rubocop:disable Layout/LineLength

    it "outputs a warning message", dev: true do
      expect(Fontist.ui).to receive(:error)
        .with("WARN: SHA256 differs (db5da6c17b02f1e6359aa8c019d9666abdf2e3438d08e77fb4b1576b6023b9f9, c5fe8a36856c7aac913b5a64cf23c9ba1afc07ac538529d61b0e08dbefd2975a)") # rubocop:disable Layout/LineLength
      formula
    end
  end

  context "contains subarchive" do
    let(:url) { "https://gitlab.com/fontmirror/langpacks/-/raw/master/OfficeLangPack2013_HE_x86.exe" } # rubocop:disable Layout/LineLength
  ...
  context "with collection which has unusual extension" do
    let(:url) { "https://gitlab.com/fontmirror/langpacks/-/raw/master/OfficeLangPack2013_KO_x86.exe" } # rubocop:disable Layout/LineLength
  ...
  context "with specified subdir option" do
    let(:url) { "https://github.com/weiweihuanghuang/Work-Sans/archive/v2.010.zip" } # rubocop:disable Layout/LineLength
  ...
  context "rpm archive" do
    let(:url) { "http://ftp.lysator.liu.se/pub/opensuse/repositories/home:/alteratio:/Common/openSUSE_13.2/src/webcore-fonts-3.0-2.1.src.rpm" } # rubocop:disable Layout/LineLength
  ...
  def expect_formula_includes(formula, example)
    ...
    exclude = %w[fonts font_collections font_version schema_version] + varies_attributes
