  ...
  describe ".from_path" do
    context "with corrupt font" do
      let(:corrupt_font_path) do
        File.join(Dir.home, "src/fontist/fontisan/spec/fixtures/fonts/Rupali/Rupali_0.72.ttf")
      end
      ...
      it "rejects corrupt font with validation error" do
        ...
        }.to raise_error(Fontist::Errors::FontFileError, /indexability validation/)
      end
      ...
      it "includes validation details in error message" do
        ...
        expect {
          described_class.from_path(corrupt_font_path)
        }.to raise_error(Fontist::Errors::FontFileError, /Table 'name' failed validation/)
      end
    end

    context "with valid font" do
      let(:fixture_path) { Fontist.root_path.join("spec", "fixtures", "fonts", "DejaVuSerif.ttf") }
